...
def parse_hopping_from_wannier90_hr_dat(filename):
    ...
    nlines = int(np.ceil(float(nrpts / 15.0)))
    ...
    t_re = hopp[:, 5]
    t_im = hopp[:, 6]
    t = t_re + 1.0j * t_im  # complex hopping amplitudes for each R, mn (H(R)_{mn})
...
def coordination_sort(atom, num_atoms, n_min, n_max, cell_vectors, positions):
    ...
    # Index filling
    index_ = np.stack(
        np.unravel_index(np.arange(num_points), (*n_size, num_atoms)), axis=-1
    )
    index_ += [*n_min, 0]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        prog="HOPPING.py",
        usage="%(prog)s input (name of input_hr.dat and input.json) output",
    )
    ...
    hops, num_wannier_funcs, n_min, n_max = parse_hopping_from_wannier90_hr_dat(
        f"{args.input_file_name}_hr.dat"
    )
    n_size = n_max - n_min + 1  # Plus 1 for 0th

    Ham_R = np.zeros(
        (n_size[0], n_size[1], n_size[2], num_wannier_funcs, num_wannier_funcs),
        dtype="c16",
    )
    ...
    for r in hops.keys():
        ...
        for m in range(num_wannier_funcs):
            for n in range(num_wannier_funcs):
                Ham_R[
                    r_idx[0] + n_max[0], r_idx[1] + n_max[1], r_idx[2] + n_max[2], m, n
                ] = Ham_idx[m, n]
    ...
    with open(args.output_file_name, "w") as fp:
        ...
        print(
            "\nAtomic positions (cart. coord.):" "\n x\t y\t z\t number of WF:", file=fp
        )
        for pos, wan in zip(positions, wanniers):
            print("{:.6f} {:.6f} {:.6f} {:.6f}".format(*pos, wan), file=fp)
        print("\nTotal number of Wannier functions: ", num_wannier_funcs, file=fp)
        print("\nPrint the complex part of the hamiltonian: ", print_complex, file=fp)
    ...
    with open(args.output_file_name, "a") as fp:
        for atom, (pos, wan) in enumerate(zip(positions, wanniers)):
            ...
            radius, index = coordination_sort(
                atom, num_atoms, n_min, n_max, cell_vectors, positions
            )
            ...
            for z, (rad, idx) in enumerate(zip(radius, index)):
                ...
                print(
                    f"Hopping <a|H|b> between atom  {atom} (000)<-->atom  {at}",
                    "(",
                    *d3,
                    f") in sphere # {sphere_num}  " "with radius  {:.6f}".format(rad),
                    f" -- {neighbor_num}:",
                    file=fp,
                )
                print("Radius vector is:  {:.6f} {:.6f} {:.6f}".format(*r), file=fp)

                move_y = wanniers[:at].sum()
                s = Ham_R[tuple(d3 + n_max)][move_x:, move_y:][:wan, : wanniers[at]]

                fmt = (
                    "({0.real:.4f}  {0.imag:.4f}i)" if print_complex else "{0.real:.6f}"
                )
                print(
                    "\n".join("  ".join(fmt.format(item) for item in row) for row in s),
                    file=fp,
                )
                print(file=fp)
